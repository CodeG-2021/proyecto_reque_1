  ...
  if (!event) {
    return (
      <div className="space-y-4">
        <h1 className="text-2xl font-semibold text-primary">Evento no encontrado</h1>
        <Button asChild>
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500">{event.type} &middot; {format(new Date(event.date), 'PPP', { locale: es })}</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 space-y-4">
          <h2 className="text-lg font-semibold text-primary">Asignación de menús</h2>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {event.assignedMenus.map((menu) => (
                <div key={menu.id} className="rounded-2xl border border-neutral-200/60 bg-white/90 p-4">
                  ...
                  <p className="mt-2 text-sm text-neutral-500">Precio estimado por invitado: ${menu.pricePerGuest}</p>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-neutral-500">Aún no se han asignado menús.</p>
          )}
        </Card>
        <Card className="space-y-4">
          <h2 className="text-lg font-semibold text-primary">Equipo asignado</h2>
          {event.staff.length ? (
            ...
            <p className="text-sm text-neutral-500">No se ha asignado personal.</p>
          )}
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="space-y-4">
          <h2 className="text-lg font-semibold text-primary">Requerimientos de inventario</h2>
          {event.inventoryNeeds.length ? (
            ...
            <p className="text-sm text-neutral-500">No hay requerimientos de inventario registrados.</p>
          )}
        </Card>
        <Card className="lg:col-span-2 space-y-4">
          <h2 className="text-lg font-semibold text-primary">Cronograma</h2>
          {event.timeline.length ? (
              ...
              {event.timeline.map((item) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span>{format(new Date(item.dueDate), 'PPP', { locale: es })}</span>
                    <span
            ...
            <p className="text-sm text-neutral-500">Los hitos del cronograma aparecerán aquí a medida que los planifiques.</p>
          )}
        </Card>
      </div>
    </div>
